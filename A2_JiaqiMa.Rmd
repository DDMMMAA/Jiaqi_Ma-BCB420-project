...
-   Note there exist and only exist two female sample (TBI 19, TBI 20) in TBI group
...
I will use the same threshold criteria utilized in [Garza et al, 2023](https://doi.org/10.1016/j.celrep.2023.113395) and split DEGs to up & down regulated genes.
...
Protein Turnover (Induced in TBI): The `Translational ubiquitination modification` and `posttranslation independent degradation` theme were strong indicator of protein turnover not identified in GSEA.
...
-   I choose `Gene Ontology` because this is the one focused in lecture. More specifically `GO:BP` because the original paper ([Garza et al, 2023](https://doi.org/10.1016/j.celrep.2023.113395)) examined `GO:BP`. The version of annotation data is `e113_eg59_p19_6be52918`
...
-   There were `1268` nodes and `12251` edges present in resulting map; `FDR q < 0.05` and `Edge Cutoff (Similarity) = 0.375` were applied.
...
-    Partially aligned with original paper ([Garza et al, 2023](https://doi.org/10.1016/j.celrep.2023.113395)). Neuroinflammation state was confirmed but activation of endogenous retroviruses in oligodendroglia was missing, presumably because of resolution difference; GSEA provde more specific pathway.

-   Link to section: [Non-thresholded Gene set Enrichment analysis, Interpretation](#GSEA-Interpretation)

2.  Can you find evidence, i.e. publications, to support some of the results that you see. How does this evidence support your result?

-    The `olfactory dysfunction`, `motile cilia dysfunction` and `neuroinflammation` were well supported by literature; `olfactory dysfunction` was supported by observing `olfactory impairment` phenotype in TBI cases clinically;  `motile cilia dysfunction` was supported by similar scRNA-seq analysis; `neuroinflammation` was supported by similar scRNA-seq and NF-kB overexpression/repression experiment on mouse model.
